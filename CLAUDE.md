...
_Last confirmed: 5 September 2026 (fifth check-in — bike crash mid-Aug + week-long bike trip = ~2.5-week layoff; iOS wiped the app's storage during the gap and history was rebuilt from check-in exports (`restore-2026-09-05.json`); towel-whip drill retired; barbell curls join the OHP bar; pulldown back-off added to pull days. **Currently in re-entry: LB and R-shoulder both flagged 2×2 post-crash — no deadlift, no overhead pressing until quiet. Next check-in = indoor-season re-cut; get the winter schedule.**) Re-confirm anything stale at the start of a session._
...
**Sport:**
- **Mon evening — squash**
- **Tue evening — volleyball** (beach in summer, indoor in winter)
- **Wed evening — volleyball**
- Daily bike commute; ~1 h total Mon–Wed, ~35 min Thu–Sun.
- **Thu evening–Sun are sport-free** (cycling aside).
...
- **Pull** (pull-ups first and fresh, chest-supported/cable rows, barbell OHP + barbell curls on one bar, lat pulldown back-off, face-pulls, Pallof/hanging raises, Copenhagen, 45 min) → **Thu or Fri.** The priority upper session — it was the one skipped in week 1; guard it.
...
- **The app coaches between check-ins (v9, Sept 2026):** a double-progression advisor shows "↑ earned a jump" when the last log tops an exercise's rep range — and shows "reps only · shoulder" on pressing while the shoulder has flagged 2 within 14 days (the amber lane, automated); ramp-up sets display computed kilos (~50%/~75% of the day's main lift); deloads are one tap (7-day mode, per-card doses, auto-expires); the More tab shows a data-health line, and sharing/copying a check-in counts as a backup. When regenerating: rep ranges in `rx` strings feed the advisor — keep them honest (top of range = jump trigger).
